 ...
 */
export function usePushNotifications(): { supported: boolean; subscribed: boolean; subscribe: () => Promise<void>; unsubscribe: () => Promise<void>; loading: boolean } {
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      const sub = await reg.pushManager.subscribe({
        ...
        applicationServerKey: (await urlBase64ToUint8Array(vapidPublicKey)).buffer as ArrayBuffer,
      });
